...
# %%
logging_file_name = input("What do you want to call the logging file?")
print(logging_file_name)

# %%
logging.basicConfig(
    filename=f'Logs/{logging_file_name if logging_file_name else "app"}.log-{datetime.now().strftime("%d-%m-%y_%H-%M-%S")}',
    filemode='w',
    level=logging.INFO,
    format='%(message)s'
)
...
agentQtable_file_name = "trained_green_wave_model.pkl"
...
#%%
from StaticAgent import RoundRobinAgent
...
for episode in range(test_episodes):
    ...
    while not done:
        action = roundRobinAgent.optimalAction(state)  # Always choose the best action
        ...
        print(f"Episode: {episode + 1:3d}| State: {state} | Action: {action} | Reward: {reward:.0f} | Total Reward: {int(total_reward):.0f}")
        
    test_rewards.append(total_reward)
    print(f"Test Episode: {episode + 1:3d} | Total Reward: {total_reward:.0f}")

# %%
avg_reward = np.mean(test_rewards[-100:])
print(f"Average rewards over 100 episodes: {avg_reward}")
